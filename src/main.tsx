...
function SummaryPage() {
  ...
  return <section className="dashboard"><div className="welcome-row"><div><p className="eyebrow orange">Panel de control</p><h2>Buenos días, María</h2><p className="muted">Aquí tienes el pulso de tus invitaciones.</p></div><Link to="/admin/invitados" className="text-link">Ver lista completa <ArrowRight size={16} /></Link></div><div className="stats-grid"><Stat icon={<Users />} value={String(total)} label="Invitados totales" detail="Lista actual" /><Stat icon={<CheckCircle2 />} value={String(confirmed)} label="Confirmados" detail={`${percent(confirmed)} de la lista`} accent /><Stat icon={<Clock3 />} value={String(pending)} label="Sin respuesta" detail={`${percent(pending)} de la lista`} /><Stat icon={<Check />} value={String(checked)} label="Ya llegaron" detail="Día del evento" /></div><div className="dashboard-columns"><div className="panel"><div className="panel-heading"><div><h3>Estado de invitaciones</h3><p className="muted">Calculado desde la lista actual</p></div><button className="icon-button"><MoreHorizontal size={18} /></button></div><div className="bar-chart"><div className="bar-row"><span>Confirmados <b>{confirmed}</b></span><div><i style={{ width: percent(confirmed) }}></i></div></div><div className="bar-row"><span>Pendientes <b>{pending}</b></span><div><i className="gray-bar" style={{ width: percent(pending) }}></i></div></div><div className="bar-row"><span>Cancelaron <b>{cancelled}</b></span><div><i className="soft-bar" style={{ width: percent(cancelled) }}></i></div></div></div></div><div className="panel next-event"><div className="panel-heading"><div><h3>Detalles del evento</h3><p className="muted">Tu próximo encuentro</p></div><CalendarDays className="orange-icon" size={20} /></div><div className="next-event-title">{event.title} <span>{event.accent}</span></div><div className="mini-detail"><CalendarDays size={16} /> {event.date}</div><div className="mini-detail"><MapPin size={16} /> {event.venue}, {event.city}</div><Link to="/admin/configuracion" className="outline-link">Editar evento <ArrowRight size={15} /></Link></div></div><div className="panel recent-panel"><div className="panel-heading"><div><h3>Actividad reciente</h3><p className="muted">Últimas respuestas registradas</p></div><Link to="/admin/invitados" className="text-link">Ver todo <ArrowRight size={15} /></Link></div><GuestRows guests={data.slice(0, 3)} /></div></section>
}
...
function GuestsPage() {
  ...
  function exportCsv() {
    const header = ['nombre', 'correo', 'celular', 'empresa', 'origen', 'invitacion', 'respuesta', 'asistencia']
    const rows = data.map(g => [g.name, g.email, g.phone, g.company, g.origin, g.invite, g.status, g.checkedIn ? 'Presente' : 'Pendiente'])
